    ...
    <div class="space-y-6" x-data="networkTreeComponent()">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-600">Susunan Network Member Anda</p>
            </div>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-1 lg:grid-cols-3 sm:gap-6">
            <x-stats-card title="Total Downline" :value="$totalDownlines" color="blue" />
            <x-stats-card title="Downline Aktif" :value="$activeDownlines" color="green" />
            <x-stats-card title="Level Terdalam" :value="$deepestLevel" color="purple" />
        </div>

        {{-- Network Tree Table --}}
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-medium text-gray-900">Network Tree (8 Levels)</h3>
                <p class="mt-1 text-sm text-gray-500">Data Netwrok Tree Anda ....!</p>
            </div>
                            ...
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div
                                            class="flex-shrink-0 h-10 w-10 flex items-center justify-center rounded-full bg-blue-100">
                                ...
                                <td class="px-6 py-4">
                                    @if (isset($networkTree[$level]) && count($networkTree[$level]) > 0)
                                        <div class="flex flex-wrap gap-3">
                                            <div class="rounded-lg border px-3 py-2 flex items-center">
                                                <a href="{{ route('pins.reedem', ['upline' => auth()->user()->id]) }}"
                                                    class="rounded p-1 text-blue-600 hover:bg-green-100"
                                            ...
                                            @foreach ($networkTree[$level] as $member)
                                                <div
                                                    class="flex items-center space-x-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
                                                    <x-member-avatar :member="$member" size="sm" />
                                                    <div class="min-w-0 flex-1">
                                                        <p class="truncate text-sm font-medium text-gray-900">
                                                            {{ $member[0]->id }}</p>
                                                        <p class="truncate text-xs text-gray-500">Upline:
                                                            {{ $member[1]->id }}</p>
                                                    </div>
                                                    ...
                                                    >
                                                        ...
                                                        @if ($level < 8)
                                                            <a href="{{ route('pins.reedem', ['upline' => $member[0]->id]) }}"
                                    ...
                                    @else
                                        <div class="text-sm text-gray-500 italic">
                                            ...
                                            @if ($level == 1)
                                                - <a href="{{ route('pins.reedem', ['upline' => auth()->user()->id]) }}"
                                                    class="text-blue-600 hover:text-blue-500">Daftarkan segera member
                                                    pertama anda</a>
                                            @endif
                                        </div>
                                    @endif
                        ...
                        <dl class="space-y-3">
                            ...
                            <div>
                                ...
                                <dd class="mt-1">
                                    <span class="inline-flex rounded-full px-2 py-1 text-xs font-semibold"
                                        :class="member.status === 'active' ? 'bg-green-100 text-green-800' :
                            ...
                            <div>
                                ...
                                <dd class="mt-1 text-sm text-gray-900"
                                    x-text="'Rp ' + new Intl.NumberFormat('id-ID').format(member.wallet_balance || 0)">
                                </dd>
                            </div>
                        </dl>
                    </div>
