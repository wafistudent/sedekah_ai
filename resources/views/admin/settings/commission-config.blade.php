...
@section('content')
    <div class="mx-auto max-w-4xl space-y-6">
        ...
        $totalCommission = 0;
        foreach ($commissionLevels as $index => $komisi) {
            $totalCommission += $komisi->amount;
        }
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                <h1 class="font-bold ">Total komisi yang didistribusikan</h1>
                <p class="mt-1 text-lg text-gray-600">Rp {{ number_format($totalCommission, 0, ',', '.') }}</p>
            </div>
        </div>
                    ...
                    @foreach ($commissionLevels as $index => $level)
                        <div class="flex items-center space-x-4 p-4 rounded-lg border border-gray-200 bg-gray-50">
                            ...
                            <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label for="level_{{ $level->level }}_amount"
                                        class="block text-sm font-medium text-gray-700">
                                        Amount (Rp)
                                    </label>
                                    <input type="number" name="levels[{{ $index }}][amount]"
                                        id="level_{{ $level->level }}_amount" min="0"
                                ...
                                <div class="flex items-end">
                                    <label class="flex items-center">
                                        <input type="checkbox" name="levels[{{ $index }}][is_active]" value="1"
