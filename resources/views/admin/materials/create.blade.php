...
@section('title', 'Add New Material - Admin')
...
<div class="mx-auto max-w-2xl space-y-6">
    ...
    {{-- Create Form --}}
    <form method="POST" action="{{ route('admin.materials.store') }}" enctype="multipart/form-data" class="space-y-6 bg-white rounded-lg shadow p-6" id="materialForm">
        @csrf
        ...
        {{-- PDF Upload Field --}}
        <div id="pdfField" class="{{ old('type', 'pdf') === 'pdf' ? '' : 'hidden' }}">
            <label for="pdf_file" class="block text-sm font-medium text-gray-700">Upload PDF <span class="text-red-500">*</span></label>
            <input 
                type="file" 
                name="pdf_file" 
                id="pdf_file" 
                ...
                "
            >
            <p class="mt-1 text-xs text-gray-500">Maximum file size: {{ $maxPdfSize }} MB</p>
            @error('pdf_file')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        {{-- YouTube URL Field --}}
        <div id="youtubeField" class="{{ old('type') === 'youtube' ? '' : 'hidden' }}">
